...
namespace reshade::d3d11
{
	...
	bool runtime_d3d11::load_effect(const reshadefx::module &module, std::string &errors)
	{
		if (_d3d_compiler == nullptr)
		...
		for (const auto &entry_point : module.entry_points)
		{
			...
			HRESULT hr = D3DCompile(module.hlsl.c_str(), module.hlsl.size(), nullptr, nullptr, nullptr, entry_point.first.c_str(), entry_point.second ? "ps_5_0" : "vs_5_0", D3DCOMPILE_ENABLE_STRICTNESS, 0, &compiled, &d3d_errors);
			...
			if (entry_point.second)
				hr = _device->CreatePixelShader(compiled->GetBufferPointer(), compiled->GetBufferSize(), nullptr, &_ps_entry_points[entry_point.first]);
			else
				hr = _device->CreateVertexShader(compiled->GetBufferPointer(), compiled->GetBufferSize(), nullptr, &_vs_entry_points[entry_point.first]);
		...
		if (constant_buffer_size != 0)
		{
			...
			const CD3D11_BUFFER_DESC globals_desc(static_cast<UINT>(constant_buffer_size), D3D11_BIND_CONSTANT_BUFFER, D3D11_USAGE_DYNAMIC, D3D11_CPU_ACCESS_WRITE);
			const D3D11_SUBRESOURCE_DATA globals_initial = { _uniform_data_storage.data() + storage_base_offset, static_cast<UINT>(constant_buffer_size) };
